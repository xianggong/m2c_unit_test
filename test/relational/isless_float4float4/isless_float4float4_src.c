...
int main(int argc, char const *argv[])
{
        ...
        /* Read program from 'isless_float4float4.cl' */
        source_code = read_buffer("isless_float4float4.cl", &source_length);
        ...
        cl_kernel kernel;
        kernel = clCreateKernel(program, "isless_float4float4", &ret);
        if (ret != CL_SUCCESS)
        {
