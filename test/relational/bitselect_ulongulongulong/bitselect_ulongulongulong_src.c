...
int main(int argc, char const *argv[])
{
        ...
        /* Read program from 'bitselect_ulongulongulong.cl' */
        source_code = read_buffer("bitselect_ulongulongulong.cl", &source_length);
        ...
        cl_kernel kernel;
        kernel = clCreateKernel(program, "bitselect_ulongulongulong", &ret);
        if (ret != CL_SUCCESS)
        {
        ...
        cl_ulong *src_2_host_buffer;
        src_2_host_buffer = malloc(num_elem * sizeof(cl_ulong));
        for (int i = 0; i < num_elem; i++)
                src_2_host_buffer[i] = (cl_ulong)(2);
        
        /* Create and init device side src buffer 2 */
        cl_mem src_2_device_buffer;
        src_2_device_buffer = clCreateBuffer(context, CL_MEM_READ_ONLY, num_elem * sizeof(cl_ulong), NULL, &ret);
        if (ret != CL_SUCCESS)
        {
                printf("error: could not create source buffer\n");
                exit(1);
        }        
        ret = clEnqueueWriteBuffer(command_queue, src_2_device_buffer, CL_TRUE, 0, num_elem * sizeof(cl_ulong), src_2_host_buffer, 0, NULL, NULL);
        if (ret != CL_SUCCESS)
        {
                printf("error: call to 'clEnqueueWriteBuffer' failed\n");
                exit(1);
