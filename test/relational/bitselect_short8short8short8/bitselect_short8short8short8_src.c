...
int main(int argc, char const *argv[])
{
        ...
        /* Read program from 'bitselect_short8short8short8.cl' */
        source_code = read_buffer("bitselect_short8short8short8.cl", &source_length);
        ...
        cl_kernel kernel;
        kernel = clCreateKernel(program, "bitselect_short8short8short8", &ret);
        if (ret != CL_SUCCESS)
        {
        ...
        cl_short8 *src_2_host_buffer;
        src_2_host_buffer = malloc(num_elem * sizeof(cl_short8));
        for (int i = 0; i < num_elem; i++)
                src_2_host_buffer[i] = (cl_short8){{2, 2, 2, 2, 2, 2, 2, 2}};
        
        /* Create and init device side src buffer 2 */
        cl_mem src_2_device_buffer;
        src_2_device_buffer = clCreateBuffer(context, CL_MEM_READ_ONLY, num_elem * sizeof(cl_short8), NULL, &ret);
        if (ret != CL_SUCCESS)
        {
                printf("error: could not create source buffer\n");
                exit(1);
        }        
        ret = clEnqueueWriteBuffer(command_queue, src_2_device_buffer, CL_TRUE, 0, num_elem * sizeof(cl_short8), src_2_host_buffer, 0, NULL, NULL);
        if (ret != CL_SUCCESS)
        {
                printf("error: call to 'clEnqueueWriteBuffer' failed\n");
                exit(1);
        ...
        cl_short8 *dst_host_buffer;
        dst_host_buffer = malloc(num_elem * sizeof(cl_short8));
        memset((void *)dst_host_buffer, 1, num_elem * sizeof(cl_short8));

        /* Create device dst buffer */
        cl_mem dst_device_buffer;
        dst_device_buffer = clCreateBuffer(context, CL_MEM_WRITE_ONLY, num_elem *sizeof(cl_short8), NULL, &ret);
        if (ret != CL_SUCCESS)
        {
        ...
        ret = clEnqueueReadBuffer(command_queue, dst_device_buffer, CL_TRUE,0, num_elem * sizeof(cl_short8), dst_host_buffer, 0, NULL, NULL);
        if (ret != CL_SUCCESS)
        {
        ...
        write_buffer(dump_file, (const char *)dst_host_buffer, num_elem * sizeof(cl_short8));
