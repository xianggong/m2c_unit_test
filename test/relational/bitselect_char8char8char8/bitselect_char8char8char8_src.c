...
int main(int argc, char const *argv[])
{
        ...
        /* Read program from 'bitselect_char8char8char8.cl' */
        source_code = read_buffer("bitselect_char8char8char8.cl", &source_length);
        ...
        cl_kernel kernel;
        kernel = clCreateKernel(program, "bitselect_char8char8char8", &ret);
        if (ret != CL_SUCCESS)
        {
        ...
        cl_char8 *src_1_host_buffer;
        src_1_host_buffer = malloc(num_elem * sizeof(cl_char8));
        for (int i = 0; i < num_elem; i++)
                src_1_host_buffer[i] = (cl_char8){{2, 2, 2, 2, 2, 2, 2, 2}};
        
        /* Create and init device side src buffer 1 */
        cl_mem src_1_device_buffer;
        src_1_device_buffer = clCreateBuffer(context, CL_MEM_READ_ONLY, num_elem * sizeof(cl_char8), NULL, &ret);
        if (ret != CL_SUCCESS)
        {
                printf("error: could not create source buffer\n");
                exit(1);
        }        
        ret = clEnqueueWriteBuffer(command_queue, src_1_device_buffer, CL_TRUE, 0, num_elem * sizeof(cl_char8), src_1_host_buffer, 0, NULL, NULL);
        if (ret != CL_SUCCESS)
        {
                printf("error: call to 'clEnqueueWriteBuffer' failed\n");
                exit(1);
        ...
        cl_char8 *src_2_host_buffer;
        src_2_host_buffer = malloc(num_elem * sizeof(cl_char8));
        for (int i = 0; i < num_elem; i++)
                src_2_host_buffer[i] = (cl_char8){{2, 2, 2, 2, 2, 2, 2, 2}};
        
        /* Create and init device side src buffer 2 */
        cl_mem src_2_device_buffer;
        src_2_device_buffer = clCreateBuffer(context, CL_MEM_READ_ONLY, num_elem * sizeof(cl_char8), NULL, &ret);
        if (ret != CL_SUCCESS)
        {
                printf("error: could not create source buffer\n");
                exit(1);
        }        
        ret = clEnqueueWriteBuffer(command_queue, src_2_device_buffer, CL_TRUE, 0, num_elem * sizeof(cl_char8), src_2_host_buffer, 0, NULL, NULL);
        if (ret != CL_SUCCESS)
        {
                printf("error: call to 'clEnqueueWriteBuffer' failed\n");
                exit(1);
