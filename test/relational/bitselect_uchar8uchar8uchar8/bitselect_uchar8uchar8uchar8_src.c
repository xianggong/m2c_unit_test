...
int main(int argc, char const *argv[])
{
        ...
        /* Read program from 'bitselect_uchar8uchar8uchar8.cl' */
        source_code = read_buffer("bitselect_uchar8uchar8uchar8.cl", &source_length);
        ...
        cl_kernel kernel;
        kernel = clCreateKernel(program, "bitselect_uchar8uchar8uchar8", &ret);
        if (ret != CL_SUCCESS)
        {
        ...
        src_2_device_buffer = clCreateBuffer(context, CL_MEM_READ_ONLY, num_elem * sizeof(cl_uchar8), NULL, &ret);
        if (ret != CL_SUCCESS)
        {
                printf("error: could not create source buffer\n");
                exit(1);
        }        
        ret = clEnqueueWriteBuffer(command_queue, src_2_device_buffer, CL_TRUE, 0, num_elem * sizeof(cl_uchar8), src_2_host_buffer, 0, NULL, NULL);
        if (ret != CL_SUCCESS)
        {
                printf("error: call to 'clEnqueueWriteBuffer' failed\n");
                exit(1);
