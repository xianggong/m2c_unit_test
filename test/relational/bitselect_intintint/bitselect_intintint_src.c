...
int main(int argc, char const *argv[])
{
        ...
        /* Read program from 'bitselect_intintint.cl' */
        source_code = read_buffer("bitselect_intintint.cl", &source_length);
        ...
        cl_kernel kernel;
        kernel = clCreateKernel(program, "bitselect_intintint", &ret);
        if (ret != CL_SUCCESS)
        {
        ...
        src_0_device_buffer = clCreateBuffer(context, CL_MEM_READ_ONLY, num_elem * sizeof(cl_int), NULL, &ret);
        if (ret != CL_SUCCESS)
        {
                printf("error: could not create source buffer\n");
                exit(1);
        }        
        ret = clEnqueueWriteBuffer(command_queue, src_0_device_buffer, CL_TRUE, 0, num_elem * sizeof(cl_int), src_0_host_buffer, 0, NULL, NULL);
        if (ret != CL_SUCCESS)
        {
        ...
        src_1_device_buffer = clCreateBuffer(context, CL_MEM_READ_ONLY, num_elem * sizeof(cl_int), NULL, &ret);
        if (ret != CL_SUCCESS)
        {
                printf("error: could not create source buffer\n");
                exit(1);
        }        
        ret = clEnqueueWriteBuffer(command_queue, src_1_device_buffer, CL_TRUE, 0, num_elem * sizeof(cl_int), src_1_host_buffer, 0, NULL, NULL);
        if (ret != CL_SUCCESS)
        {
                printf("error: call to 'clEnqueueWriteBuffer' failed\n");
                exit(1);
        ...
        cl_int *src_2_host_buffer;
        src_2_host_buffer = malloc(num_elem * sizeof(cl_int));
        for (int i = 0; i < num_elem; i++)
                src_2_host_buffer[i] = (cl_int)(2);
        
        /* Create and init device side src buffer 2 */
        cl_mem src_2_device_buffer;
        src_2_device_buffer = clCreateBuffer(context, CL_MEM_READ_ONLY, num_elem * sizeof(cl_int), NULL, &ret);
        if (ret != CL_SUCCESS)
        {
                printf("error: could not create source buffer\n");
                exit(1);
        }        
        ret = clEnqueueWriteBuffer(command_queue, src_2_device_buffer, CL_TRUE, 0, num_elem * sizeof(cl_int), src_2_host_buffer, 0, NULL, NULL);
        if (ret != CL_SUCCESS)
        {
                printf("error: call to 'clEnqueueWriteBuffer' failed\n");
                exit(1);
