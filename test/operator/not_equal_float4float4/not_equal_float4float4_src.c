...
int main(int argc, char const *argv[])
{
        ...
        /* Read program from 'not_equal_float4float4.cl' */
        source_code = read_buffer("not_equal_float4float4.cl", &source_length);
        ...
        cl_kernel kernel;
        kernel = clCreateKernel(program, "not_equal_float4float4", &ret);
        if (ret != CL_SUCCESS)
        {
