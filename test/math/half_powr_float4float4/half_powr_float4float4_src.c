...
int main(int argc, char const *argv[])
{
        ...
        /* Read program from 'half_powr_float4float4.cl' */
        source_code = read_buffer("half_powr_float4float4.cl", &source_length);
        ...
        cl_kernel kernel;
        kernel = clCreateKernel(program, "half_powr_float4float4", &ret);
        if (ret != CL_SUCCESS)
        {
