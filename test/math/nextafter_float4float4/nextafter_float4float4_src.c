...
int main(int argc, char const *argv[])
{
        ...
        /* Read program from 'nextafter_float4float4.cl' */
        source_code = read_buffer("nextafter_float4float4.cl", &source_length);
        ...
        cl_kernel kernel;
        kernel = clCreateKernel(program, "nextafter_float4float4", &ret);
        if (ret != CL_SUCCESS)
        {
