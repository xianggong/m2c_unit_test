...
int main(int argc, char const *argv[])
{
        ...
        /* Read program from 'clamp_float4float4float4.cl' */
        source_code = read_buffer("clamp_float4float4float4.cl", &source_length);
        ...
        cl_kernel kernel;
        kernel = clCreateKernel(program, "clamp_float4float4float4", &ret);
        if (ret != CL_SUCCESS)
        {
